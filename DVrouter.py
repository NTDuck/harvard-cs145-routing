...
class DVrouter(Router):
    ...
    def handle_packet(self, port: _Port, packet: Packet):
        ...
        if packet.is_traceroute:
            ...
            #   send packet based on forwarding table, e.g., self.send(port, packet)
                ...
                if entry.cost < _INFINITY and entry.port is not None:
            ...
            #   update the forwarding table
                    ...
                    new_neighbor_cost = min(entry.cost + neighbor_cost, _INFINITY) # Bellman-Ford algorithm
    ...
    def __broadcast_to_neighbors(self):
        for neighbor_addr, entry in self.__neighbors_by_addrs.items():
            distance_vector = {addr: _DistanceVectorEntry(cost=_INFINITY if neighbor_addr != addr and neighbor_addr == entry.next_hop else entry.cost, next_hop=entry.next_hop) for addr, entry in self.__forwarding_table.items()} # Poisoned reverse
